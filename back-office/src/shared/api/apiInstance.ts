    ...
    async (error: AxiosError) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry && !isAuthRequest) {
            ...
            try {
                ...
                console.log('✅ [Auth Rotation] Silent refresh successful. Session extended.');
                return api(originalRequest);
            } catch (refreshError: any) {
                console.error('❌ [Auth Rotation] Critical failure during token rotation:', refreshError);
                processQueue(refreshError, null);

                // If refresh fails, it might be a TOKEN_REPLAY or EXPIRED
                const errorMsg = refreshError.response?.data?.message || '';
                if (errorMsg.includes('COMPROMISE') || errorMsg.includes('PANIC') || refreshError.response?.status === 403) {
                    console.warn('🚨 [Security Audit] Possible compromise detected. Revoking local identity.');
                    clearAuth();
