...
export const CustomerProfileDrawer: React.FC<CustomerProfileDrawerProps> = ({
  ...
  const {
    ...
    queryFn: () =>
      customerId ? customerApi.getCustomerDetail(customerId) : null,
  ...
  const handleRevealPII = () => {
    if (!customer) return;
    setPiiVisible(true);
    customerApi.logPiiAccess(
      customer.id,
      "Manual reveal for clienteling check",
    );
      ...
      {isLoading ? (
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-right-10 duration-500">
          ...
          <div className="flex justify-between items-start">
            <Space size={20} align="start">
              <Avatar
                ...
                icon={<UserOutlined />}
                className="border-2 border-gold-200 shadow-sm"
              />
              <div>
                ...
                <Space size={8} wrap>
                  <Tag
                    color={isVIP ? "gold" : "blue"}
                    className="text-[10px] uppercase font-bold tracking-widest border-none px-3 py-0.5"
                  >
                    {isVIP ? <CrownOutlined className="mr-1" /> : null}
                    {segment}
                  </Tag>
                  <Tag className="text-[10px] uppercase tracking-widest m-0 px-2">
                    Member since {new Date(customer.createdAt).getFullYear()}
                  </Tag>
                  <Badge
                    status={customer.isActive ? "success" : "default"}
                    text={
                      <Text className="text-[10px] uppercase text-gray-400 font-bold">
                        Account {customer.isActive ? "Active" : "Disabled"}
                      </Text>
                    }
                  />
                </Space>
            ...
            <div className="text-right">
              <Text className="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-1">
                Lifetime Value
              </Text>
              <Title
                level={2}
                className="!mb-0 !mt-0 font-light text-amber-600"
              >
          ...
          <Row gutter={16}>
            <Col span={8}>
              <Card className="bg-gray-50 border-none shadow-none text-center rounded-none overflow-hidden hover:bg-white hover:shadow-sm transition-all cursor-default">
                <Statistic
                  title={
                    <Text className="text-[10px] tracking-[0.2em] font-bold uppercase text-gray-400">
                      Total Orders
                    </Text>
                  }
                  value={orderCount}
                ...
                />
                <div className="mt-2 text-[10px] text-gray-400 uppercase font-bold">
                  Frequency: {orderCount > 5 ? "High" : "Moderate"}
                </div>
              </Card>
            </Col>
            <Col span={8}>
            ...
            title={
              <Space className="w-full justify-between">
                ...
                <Button
                  ...
                  onClick={
                    piiVisible ? () => setPiiVisible(false) : handleRevealPII
                  }
                  className={
                    piiVisible
                      ? "text-gray-400"
                      : "bg-black border-none text-[10px] font-bold uppercase px-3"
                  }
                >
            ...
            {!piiVisible && (
              <div className="mt-4 p-2 bg-blue-50/50 flex items-start gap-2 rounded border border-blue-100/50">
                ...
                <Text className="text-[10px] text-blue-500">
                  This information is masked for data privacy. Revealing it will
                  generate an audit entry linked to your account.
                </Text>
              </div>
            )}
          </Card>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Card
                ...
                className="shadow-sm border-gray-100 rounded-none h-full"
              >
                      ...
                      <List.Item className="px-0 border-none pb-3">
                        <div className="w-full">
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <Text className="text-[10px] text-amber-600 font-serif">
                              {p.value}
                            </Text>
                          </div>
                          <Progress
                      ...
                      <List.Item className="px-0 border-b border-gray-50 last:border-none py-2">
                        <Space size={12} align="start">
                          ...
                          <div>
                            <Text
                              strong
                              className="text-xs uppercase tracking-tight block"
                            >
                              {e.eventType}
                            </Text>
          ...
          <div>
            <Divider
              orientation="left"
              className="!text-xs !uppercase !tracking-widest !text-gray-400 font-bold mb-6"
            >
              Clienteling Timeline
            </Divider>
            <Timeline
              mode="left"
              items={[
                ...customer.orders.map((o: any) => ({
                  ...
                  children: (
                    <Card
                      size="small"
                      className="border-l-4 border-l-blue-500 shadow-sm rounded-none hover:shadow-md transition-shadow cursor-pointer"
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <Text
                            ...
                            className="text-xs uppercase tracking-tight block"
                          >
                            Order Placed #{o.orderNumber || o.id.slice(0, 8)}
                          </Text>
                          <Text className="text-xs text-gray-500 font-light block mt-1">
                            {o.items?.length || 1} pieces — Status:{" "}
                            <Tag
                              color="processing"
                        ...
                        <Text className="font-serif text-sm font-medium">
                  ...
                  children: (
                    <div className="flex items-center gap-2">
                      ...
                      <Text className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                        Account Created at Ray Paradis
                      </Text>
                    </div>
                  ),
                  color: "gray",
                ...
                const dateA = new Date(
                  a.label.props.children as string,
                ).getTime();
                const dateB = new Date(
                  b.label.props.children as string,
                ).getTime();
                return dateB - dateA;
              })}
            />
          ...
          <Card
            ...
            className="shadow-sm border-gray-100 rounded-none bg-amber-50/20"
          >
            <Paragraph className="text-xs text-gray-600 font-light italic">
              {customer.bio ||
                "No personalized notes yet. Add preferences here e.g. 'Loves Rose Gold', 'Buying for 10th anniversary'."}
            </Paragraph>
            <Button
              type="dashed"
              block
              className="mt-2 text-[10px] font-bold uppercase tracking-widest border-gray-300 h-10 hover:border-black hover:text-black transition-colors"
            >
              Update Journal
            </Button>
          </Card>
