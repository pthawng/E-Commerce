...
import { Layout, Menu, Button, Typography, Space, Input, Badge, Dropdown, Avatar, MenuProps } from 'antd';
import {
    DashboardOutlined,
    AuditOutlined,
    ShoppingOutlined,
...
export const AppLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const quickActionItems: MenuProps['items'] = [
        {
            key: 'new-order',
            label: <span className="text-[10px] uppercase tracking-widest font-bold">{t('common.forge_order')}</span>,
            icon: <PlusOutlined className="text-[10px]" />,
        },
        {
            key: 'add-material',
            label: <span className="text-[10px] uppercase tracking-widest font-bold">{t('common.intake_material')}</span>,
        ...
        {
            key: 'qc-approval',
            label: <span className="text-[10px] uppercase tracking-widest font-bold">{t('common.qc_approval')}</span>,
    ...
    const regionalItems: MenuProps['items'] = [
        {
            ...
            onClick: () => { i18n.changeLanguage('vi'); setLocale('vi'); }
        },
        {
            key: 'en',
            label: 'English (USD)',
        ...
        },
        {
            key: 'zh',
            label: '中文 (CNY)',
            onClick: () => { i18n.changeLanguage('zh'); setLocale('zh'); }
        },
    ];
            ...
            >
                <div className="h-20 flex items-center px-6 mb-8 border-b border-gray-50">
                    <div className="flex items-center gap-3">
                        ...
                        {!collapsed && (
                            <Text className="font-serif italic tracking-[0.1em] text-lg">Ray Paradis</Text>
                        )}
                    </div>
                </div>
                ...
                <div className="px-3">
                    {!collapsed && (
                        <Text className="text-[9px] font-bold tracking-[0.2em] text-gray-400 uppercase px-4 block mb-4">{t('common.modules', { defaultValue: 'Modules' })}</Text>
                    )}
                    <Menu
                        mode="inline"
                ...
                {!collapsed && (
                    <div className="absolute bottom-8 left-0 right-0 px-6">
                        <div className="p-4 bg-gray-50 border border-gray-100">
                            <Text className="text-[9px] uppercase tracking-widest text-gray-400 block mb-1">{t('common.system_status', { defaultValue: 'System Status' })}</Text>
                            <div className="flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]" />
                                <Text className="text-[10px] font-bold tracking-tighter">{t('common.vault_connected', { defaultValue: 'VAULT CONNECTED' })}</Text>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <Header className="!bg-white border-b border-gray-100 h-24 px-8 flex items-center justify-between sticky top-0 z-50 overflow-hidden">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center min-w-0 px-8">
                        <AnimatePresence mode="wait">
                            <motion.div
                                key={location.pathname + (currentHeader?.title?.toString() || '')}
                            ...
                            >
                                ...
                                <div className="relative group">
                                    ...
                                    <motion.div
                                        className="absolute -bottom-1 left-1/2 -translate-x-1/2 h-[1px] bg-black/10 w-0 group-hover:w-full transition-all duration-500"
                                    />
                                </div>
                                ...
                                {(currentHeader?.subtitle || currentHeader?.isLoading) && (
                                    <Text className="text-[10px] text-gray-400 uppercase tracking-[0.15em] font-medium mt-1 truncate max-w-sm">
                                        {currentHeader?.isLoading ? 'Synchronizing Vault...' : currentHeader?.subtitle}
                                    </Text>
                                )}
                            </motion.div>
                        </AnimatePresence>
