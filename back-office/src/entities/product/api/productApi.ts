...
export const productApi = {
    ...
    getProduct: (id: string) => api.get<ProductDetail>(`/products/${id}`).then(res => res.data),

    getProductBySlug: (slug: string) => api.get<ProductDetail>(`/products/slug/${slug}`).then(res => res.data),

    createProduct: (data: FormData) => api.post<ProductDetail>('/admin/products', data).then(res => res.data),

    updateProduct: (id: string, data: FormData) => api.patch<ProductDetail>(`/admin/products/${id}`, data).then(res => res.data),
    ...
    uploadMedia: (productId: string, file: File, options?: { isThumbnail?: boolean; order?: number }) => {
        ...
        return api
            .post<ProductMedia>(`/products/${productId}/media`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            })
            .then(res => res.data);
    },

    deleteMedia: (productId: string, mediaId: string) => api.delete(`/products/${productId}/media/${mediaId}`),

    setThumbnail: (productId: string, mediaId: string) => api.patch(`/products/${productId}/media/${mediaId}/thumbnail`),
};
