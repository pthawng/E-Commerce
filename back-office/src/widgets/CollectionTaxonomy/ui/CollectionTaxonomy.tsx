...
export const CollectionTaxonomy: React.FC = memo(() => {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) =>
      message.error(
        err.response?.data?.message || t("dashboard.integrity.audit_failed"),
      ),
  });
  ...
  const buildTreeData = (nodes: CategoryTreeNode[]): any[] => {
    return nodes.map((node) => ({
      ...
      title: (
        <div className="flex items-center justify-between group w-full pr-4">
          ...
          <Space size={8}>
            ...
            {!node.isActive && (
              <Tag color="default" className="text-[8px] m-0">
                {t("common.inactive")}
              </Tag>
            )}{" "}
            {node.path?.startsWith("collections") && (
              <Tag color="purple" className="text-[8px] m-0">
                COLLECTION
              </Tag>
            )}{" "}
          </Space>{" "}
      ...
      icon: node.children?.length ? (
        ...
        <FolderOutlined className="text-gray-300" />
      ),
    }));
  };
  return (
      ...
      <Modal
        title={
          <span className="font-serif">
            {t("products.categories.create_modal")}
          </span>
        }
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              ...
              <Form.Item name="order" label={t("products.categories.order")}>
                ...
                <InputNumber
                  min={0}
                  className="w-full rounded-none h-10 leading-10"
                />{" "}
              </Form.Item>{" "}
            </Col>{" "}
            <Col span={12}>
              ...
              >
                {" "}
                <Switch
                  checkedChildren={t("inventory.status.active")}
