...
export const AttributeRegistry: React.FC = memo(() => {
  ...
  const filterTypeBadge = (type: string) => {
    ...
    return (
      <Tag
        color={info.color}
        className="text-[8px] uppercase tracking-widest m-0"
      >
        {info.label}
      </Tag>
    );
  ...
  const technical =
    attributes?.filter((a) => technicalCodes.includes(a.code)) || [];
  const marketing =
    attributes?.filter((a) => !technicalCodes.includes(a.code)) || [];
          ...
          items={items.map((attr) => ({
            ...
            label: (
              <div className="flex items-center justify-between w-full pr-4">
                ...
                <Space size={4}>
                  {" "}
                  <Tag className="text-[8px] m-0">
                    {attr.values?.length || 0} values
                  </Tag>{" "}
                  <Popconfirm
                    title={t("common.delete") + "?"}
                    onConfirm={() => deleteMutation.mutate(attr.id)}
                  >
                    {" "}
                    <Button
                      type="text"
              ...
              <div className="pl-6">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {attr.values?.map((v) => (
                    <Tag
                      key={v.id}
                      className="text-xs py-1 px-3 rounded-sm border-gray-200 dark:border-gray-800"
                    >
                      {" "}
                      {attr.filterType === "swatch_color" && v.metaValue && (
                        <span
                          className="inline-block w-3 h-3 rounded-full mr-2 border dark:border-gray-700"
                          style={{ backgroundColor: v.metaValue }}
                        />
                      )}{" "}
                      {getName(v.value)}{" "}
                      {v.metaValue && (
                        <Text type="secondary" className="ml-2 text-[9px]">
                          ({v.metaValue})
                        </Text>
                      )}{" "}
                    </Tag>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          {renderAttributeGroup(
            t("products.attributes.technical"),
            <SafetyCertificateOutlined className="text-blue-500" />,
            technical,
          )}{" "}
          {renderAttributeGroup(
            t("products.attributes.marketing"),
      ...
      <Modal
        title={
          <span className="font-serif">
            {t("products.attributes.create_modal")}
          </span>
        }
      ...
      >
        ...
        >
          ...
          <Form.Item
            ...
            rules={[
              { required: true },
              { pattern: /^[a-z_]+$/, message: "Lowercase + underscore only" },
            ]}
          >
            {" "}
            <Input
              placeholder="stone_color"
              className="rounded-none h-10"
            />{" "}
          </Form.Item>{" "}
          <Form.Item
            name="nameVi"
            label={t("products.categories.name_vi")}
            rules={[{ required: true }]}
          >
            {" "}
