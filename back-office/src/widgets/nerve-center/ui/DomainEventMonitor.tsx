...
import { 
    Table, Tag, Space, Typography, Tooltip, 
    Row, Col, Card, Statistic 
} from 'antd';
import { 
    DatabaseOutlined, BugOutlined, EyeOutlined, 
    PlayCircleOutlined 
...
export const DomainEventMonitor: React.FC = memo(() => {
    ...
    const columns = [
        {
            ...
            dataIndex: 'eventType',
            key: 'eventType',
            render: (v: string) => <Text className="font-mono text-[10px] bg-gray-50 dark:bg-white/5 px-2 py-1 border border-gray-100 dark:border-gray-800 uppercase tracking-widest">{v}</Text>
        },
        {
        ...
        },
        {
            ...
            render: (_: any, record: any) => (
                <Space size={12}>
                    <Tooltip title="View Payload"><EyeOutlined className="text-gray-400 cursor-pointer hover:text-black dark:hover:text-white" /></Tooltip>
    ...
    return (
        <div className="space-y-6">
            <Row gutter={16}>
                <Col span={6}>
                    <Card size="small" className="bg-gray-50/50 dark:bg-white/5 border-gray-100 dark:border-gray-900 rounded-none shadow-none">
                        <Statistic
                            title={<Text className="text-[9px] uppercase tracking-widest font-bold text-gray-400">{t('nerve_center.events.backlog')}</Text>}
                            value={12}
                ...
                <Col span={6}>
                    <Card size="small" className="bg-gray-50/50 dark:bg-white/5 border-gray-100 dark:border-gray-900 rounded-none shadow-none">
                        <Statistic
                            title={<Text className="text-[9px] uppercase tracking-widest font-bold text-gray-400">{t('nerve_center.events.failure_rate')}</Text>}
                            value={0.8}
                            suffix="%"
