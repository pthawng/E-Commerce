...
import { Badge, Button, Dropdown, List, Avatar, Typography, Space, Empty, Tag, Divider } from 'antd';
import { BellOutlined, CheckCircleOutlined, InfoCircleOutlined, WarningOutlined, CloseCircleOutlined, ThunderboltOutlined, RightOutlined } from '@ant-design/icons';
...
export const NotificationCenter: React.FC = () => {
    const { notifications, unreadCount, init, markAsRead, markAllAsRead, isLoading } = useNotificationStore();
    ...
    const getIcon = (type: string) => {
        switch (type) {
            ...
            case 'ACTION_REQUIRED': return <ThunderboltOutlined className="text-purple-500" />;
    ...
    const notificationContent = (
        <div className="w-[380px] bg-white shadow-2xl border border-gray-100 overflow-hidden luxury-dropdown">
            <div className="px-6 py-5 flex items-center justify-between border-b border-gray-50 bg-gray-50/50">
                <Title level={5} className="!mb-0 font-serif italic text-lg">Nerve Center</Title>
                <Space>
                    {unreadCount > 0 && (
                        <Button type="text" size="small" className="text-[10px] font-bold tracking-widest uppercase hover:text-black" onClick={() => markAllAsRead()}>
                            Mark all read
                        </Button>
                    )}
                </Space>
            ...
            <div className="max-h-[450px] overflow-auto">
                {notifications.length === 0 ? (
                    <div className="py-12 flex flex-col items-center justify-center opacity-40">
                        ...
                        <Text className="text-[10px] uppercase tracking-widest mt-4">System is silent</Text>
                    </div>
                ) : (
                    <List
                                ...
                                <List.Item.Meta
                                    ...
                                    title={
                                        <div className="flex items-center justify-between">
                                            <Text className={`text-[11px] font-bold tracking-tight uppercase ${!n.isRead ? 'text-black' : 'text-gray-400'}`}>
                                                {n.title}
                                            </Text>
                                    ...
                                    description={
                                        <div className="flex flex-col gap-2 mt-1">
                                            ...
                                            {n.metadata?.actionLabel && (
                                                ...
                                                >
                                                    {n.metadata.actionLabel} <RightOutlined className="text-[8px]" />
                                                </Button>
                                            )}
                                        </div>
                                    }
            ...
            <div className="p-3 border-t border-gray-50 bg-gray-50/30 flex justify-center">
                <Button type="text" className="text-[10px] font-bold tracking-[0.2em] uppercase text-gray-400 hover:text-black">
                    View System Logs
                </Button>
            </div>
        </div>
