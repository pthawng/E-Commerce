...
import { Table, Tag, Avatar, Space, Button, Badge, Tabs, Typography, Progress, Tooltip, Dropdown, MenuProps } from 'antd';
import { 
...
import { OrderListItem } from '@/shared/api/orderApi';
...
export const OrderOrchestrator: React.FC<OrderOrchestratorProps> = memo(({
    ...
    const getUrgencySignal = (record: OrderListItem) => {
        ...
        if (record.status !== OrderStatusEnum.COMPLETED && record.status !== OrderStatusEnum.CANCELLED) {
            if (hoursOld > 48) return <Tooltip title={t('dashboard.issues')}><AlertOutlined className="text-red-500 animate-pulse" /></Tooltip>;
            if (hoursOld > 12) return <Tooltip title={t('dashboard.pipeline_alerts')}><ClockCircleOutlined className="text-orange-400" /></Tooltip>;
        }
        return null;
    };
    ...
    const columns = useMemo(() => [
            ...
            render: (_: any, record: OrderListItem) => (
                <Space size="middle">
                    ...
                    <div className="flex flex-col">
                        <Text className="text-[11px] font-bold tracking-widest text-[#0e2258] dark:text-[#d4af37] uppercase">{record.code}</Text>
        ...
        },
        {
            ...
            key: 'client',
            render: (_: any, record: OrderListItem) => {
                const segment = record.user?.segment || LuxurySegment.PROSPECT;
                const config = LUXURY_SEGMENT_CONFIG[segment];
                return (
                    <Space size="middle">
                        <Badge dot={segment === LuxurySegment.VIC} color="red">
                            <Avatar size="small" shape="square" className="bg-gray-100 text-[9px] font-bold text-gray-400">
                                {record.user?.fullName?.[0] || 'G'}
                            </Avatar>
                        </Badge>
                        <div className="flex flex-col" onClick={() => onViewOrder(record.id)} style={{ cursor: 'pointer' }}>
                            <div className="flex items-center gap-2">
                                <Text className="text-xs font-serif font-medium">{record.user?.fullName || record.guestFullName || t('dashboard.patron_default')}</Text>
                ...
                return (
                    <div className="flex flex-col gap-1 w-40">
                        <div className="flex justify-between items-center">
                            ...
                            <Text className="text-[9px] text-gray-300 italic">{getProgress(record.status)}%</Text>
                        </div>
                        <Progress percent={getProgress(record.status)} showInfo={false} size="small" strokeColor={config.color === 'default' ? '#d9d9d9' : undefined} />
                    </div>
                );
            }
        },
        {
            ...
            key: 'finance',
            render: (_: any, record: OrderListItem) => {
                const config = PAYMENT_STATUS_CONFIG[record.paymentStatus as PaymentStatusEnum] || PAYMENT_STATUS_CONFIG.unpaid;
                return (
                    <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <Text className={`text-[9px] uppercase tracking-widest font-bold ${record.paymentStatus === 'paid' ? 'text-green-600' : 'text-red-500'}`}>
                                {config.label}
                            </Text>
                        </div>
                    </div>
        ...
        },
        {
            ...
            render: (_: any, record: OrderListItem) => {
                ...
                const menuItems: MenuProps['items'] = transitions.map(status => ({
                    key: status,
                    label: `${t('common.edit')} -> ${ORDER_STATUS_CONFIG[status].label}`,
                    onClick: (e: any) => {
                        e.domEvent.stopPropagation();
                        onTransition(record.id, status);
                    }
                }));
                    ...
                    <Space size="small">
                        {transitions.length > 0 ? (
                            <Button 
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    onTransition(record.id, transitions[0]);
                                }}
                            >
                                {ORDER_STATUS_CONFIG[transitions[0]].label}
                            </Button>
                        ) : (
                            <Button size="small" icon={<CheckCircleOutlined />} disabled className="text-[9px] h-7">{t('dashboard.archival_intelligence')}</Button>
                        )}
    ...
    const items = [
        ...
        { 
            key: 'critical', 
            label: <span className="px-4 uppercase tracking-[0.2em] text-[10px] font-bold flex items-center gap-2"><Badge dot status="error" /> {t('orders.queue.critical')}</span>,
        },
        { 
            key: 'production', 
            label: <span className="px-4 uppercase tracking-[0.2em] text-[10px] font-bold">{t('orders.queue.production')}</span>,
        },
    ];
