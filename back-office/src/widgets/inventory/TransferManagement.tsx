...
import { Table, Tag, Space, Button, Modal, Form, Select, InputNumber, Input, message, Typography } from 'antd';
import { 
    ArrowRightOutlined, 
    CheckCircleOutlined,
    ClockCircleOutlined,
...
import { inventoryApi, InventoryTransfer } from '@/shared/api/inventoryApi';
...
export const TransferManagement: React.FC = () => {
    ...
    const getStatusTag = (status: string) => {
        ...
        return (
            <Tag icon={icons[status]} color={colors[status]} className="rounded-none font-bold text-[10px]">
                {status}
            </Tag>
        );
    };
    ...
    const columns = [
        {
            ...
            dataIndex: 'id',
            key: 'id',
            render: (id: string) => <Text code className="text-[10px]">{id.slice(0, 8)}</Text>,
        },
        {
        ...
        },
        {
            ...
            render: (sku: string, record: InventoryTransfer) => (
                <Text className="text-xs">{sku || record.productVariant?.sku || 'N/A'}</Text>
            ),
        },
        {
    ...
    return (
        <div className="space-y-6">
            ...
            <Modal
                title={<Title level={5} className="font-serif !mb-0">{t('inventory.transfers.initiate')}</Title>}
                open={isModalOpen}
            ...
            >
                <Form form={form} layout="vertical" onFinish={(values) => createMutation.mutate(values)} className="mt-4">
                    <Form.Item name="variantId" label={t('inventory.transfers.form.variant')} rules={[{ required: true }]}>
                        <Select 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Form.Item name="fromWarehouseId" label={t('inventory.transfers.from')} rules={[{ required: true }]}>
                            <Select placeholder={t('common.search_placeholder')} options={warehouseOptions} className="luxury-select" />
                        </Form.Item>
                        <Form.Item name="toWarehouseId" label={t('inventory.transfers.to')} rules={[{ required: true }]}>
                            <Select placeholder={t('common.search_placeholder')} options={warehouseOptions} className="luxury-select" />
                        </Form.Item>
                    </div>
                    <Form.Item name="quantity" label="Quantity" rules={[{ required: true }]} initialValue={1}>
                        <InputNumber min={1} className="w-full h-10 leading-10 rounded-none" />
                    </Form.Item>
                    <Form.Item name="note" label={t('inventory.transfers.form.note')}>
                        <TextArea placeholder={t('inventory.transfers.form.note_placeholder')} className="rounded-none" rows={3} />
                    </Form.Item>
                </Form>
            </Modal>
        </div>
