...
import { Table, Tag, Space, Input, Select, Card, Typography, Tooltip, Badge, Button } from 'antd';
import { 
    SearchOutlined, 
    FilterOutlined, 
    ReloadOutlined,
...
import { inventoryApi, InventoryItem } from '@/shared/api/inventoryApi';
...
export const StockTable: React.FC = memo(() => {
    ...
    const { data: stock, isLoading, refetch } = useQuery({
        ...
        queryFn: () => inventoryApi.getStockLevels({ warehouseId: warehouseFilter }),
    });
    ...
    const columns = useMemo(() => [
            ...
            render: (_: any, record: InventoryItem) => (
                <div className="flex flex-col">
                    <Text strong className="font-serif text-sm">
                        {record.productVariant?.name?.vi || record.productVariant?.sku || t('common.no_data')}
                    </Text>
        ...
        },
        {
            ...
            align: 'right' as const,
            render: (_: any, record: InventoryItem) => {
                const available = (record.quantity || 0) - (record.reservedQuantity || 0) - (record.damagedQuantity || 0);
                return (
                    <Badge 
                        count={available} 
                        overflowCount={9999}
                        style={{ backgroundColor: available <= 5 ? '#f5222d' : '#52c41a' }}
                        className="font-bold"
                    />
                );
            },
        },
        {
            ...
            render: (qty: number) => (
                <Space size={4}>
                    {qty > 0 && <SwapOutlined className="text-blue-400 animate-pulse" />}
                    <Text type={qty > 0 ? undefined : 'secondary'} className={`text-sm ${qty > 0 ? 'text-blue-600 font-bold' : ''}`}>
                        {qty > 0 ? `+${qty}` : '0'}
                    </Text>
                </Space>
            ),
        },
        {
            ...
            key: 'shelf',
            render: (loc: string) => (
                <Text code className="text-[10px]">{loc || 'N/A'}</Text>
            ),
        },
    ], [t, convertAndFormat]);
        ...
        <div className="space-y-6">
            ...
            <Table
                ...
                summary={(pageData) => {
                    ...
                    pageData.forEach(({ quantity, reservedQuantity, damagedQuantity }) => {
                        totalAvailable += ((quantity || 0) - (reservedQuantity || 0) - (damagedQuantity || 0));
                    });
                    return (
                        <Table.Summary.Row className="bg-gray-50 dark:bg-white/5 font-bold">
                            <Table.Summary.Cell index={0} colSpan={5}>{t('inventory.stock.salable_total')}</Table.Summary.Cell>
            ...
            <Card size="small" className="bg-amber-50/30 dark:bg-amber-900/10 border-amber-100 dark:border-amber-900/30">
                <Space>
                    ...
                    <Text className="text-[11px] text-amber-700 dark:text-amber-400">
                        ...
                        <Text underline className="ml-2 cursor-pointer">{t('inventory.stock.view_drift')}</Text>
                    </Text>
                </Space>
            </Card>
        </div>
