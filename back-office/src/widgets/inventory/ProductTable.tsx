...
import { productApi, ProductListItem, ProductVariantSummary } from '@/shared/api/productApi';
...
export const ProductTable: React.FC = () => {
    ...
    const columns: ColumnsType<ProductListItem> = [
        ...
        {
            title: t('inventory.table.variants'),
            dataIndex: 'variants',
            key: 'variants',
        ...
        },
        {
            ...
            key: 'status',
            render: (_, record) => (
                <Tag color={record.isActive ? 'success' : 'default'} className="rounded-none border-none text-[10px] uppercase font-bold px-2 py-0">
                    {record.isActive ? t('inventory.status.active') : t('inventory.status.draft')}
                </Tag>
            ),
        },
        {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-serif mb-1">{t('inventory.title')}</h2>
                    <p className="text-xs text-gray-400 italic">{t('inventory.subtitle')}</p>
                </div>
                <Button type="primary" className="h-10 px-8 bg-black text-white hover:bg-gray-800 border-none uppercase tracking-widest text-[9px] font-bold">
            ...
            <div className="flex flex-wrap gap-4 items-center bg-gray-50/50 p-4 border border-gray-100">
                ...
                <div className="ml-auto text-[10px] text-gray-400 uppercase tracking-widest font-medium">
                    {t('inventory.products_found', { count: total })}
                </div>
            </div>
