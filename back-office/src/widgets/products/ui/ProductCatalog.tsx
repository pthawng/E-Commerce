...
import { 
    Table, Tag, Space, Button, Input, Badge, Tooltip, 
    Typography, message, Drawer, Descriptions, Divider, Card 
} from 'antd';
import { 
    PlusOutlined, SearchOutlined, EyeOutlined, 
    GoldOutlined, StarFilled, CheckCircleOutlined 
...
export const ProductCatalog: React.FC = memo(() => {
    ...
    const columns = [
            ...
            render: (_: any, record: ProductListItem) => (
                <Space size={4} wrap>
                    {record.categories?.map(c => (
                        <Tag key={c.categoryId} className="text-[9px] uppercase tracking-widest m-0 border-gray-200 dark:border-gray-800">
        ...
        },
        {
            ...
            key: 'price',
            render: (_: any, record: ProductListItem) => {
                if (!record.displayPriceMin) return <Text type="secondary">{t('products.catalog.table.bespoke')}</Text>;
                    ...
                    <div className="space-y-6">
                        <Descriptions title={t('products.catalog.drawer.identity')} bordered column={2} size="small">
                            ...
                            <Descriptions.Item label={t('common.status')}>
                                <Badge status={selectedProduct.isActive ? 'success' : 'default'} text={selectedProduct.isActive ? t('inventory.status.active') : t('common.draft')} />
                            </Descriptions.Item>
                            <Descriptions.Item label="Featured">
                                {selectedProduct.isFeatured ? <StarFilled className="text-amber-400" /> : 'No'}
                            </Descriptions.Item>
                            <Descriptions.Item label={t('orders.drawer.date')}>{new Date(selectedProduct.createdAt).toLocaleDateString()}</Descriptions.Item>
                        </Descriptions>

                        {selectedProduct.description && (
                            <Card size="small" title={t('profile.bio')} className="rounded-none border-gray-100 dark:border-gray-900 shadow-none">
                                <Paragraph className="text-xs text-gray-500">{getName(selectedProduct.description)}</Paragraph>
                            </Card>
                        )}

                        <Divider orientation="left" className="!text-xs !uppercase !tracking-widest !text-gray-400">{t('products.catalog.drawer.variants_count', { count: selectedProduct.variants?.length || 0 })}</Divider>
                        <Table
                            ...
                            columns={[
                                ...
                                {
                                    title: t('products.categories.name_en'), render: (_: any, r: any) => {
                                ...
                                { title: t('products.catalog.table.valuation'), dataIndex: 'price', render: (v: number) => convertAndFormat(v) },
                        ...
                        {selectedProduct.media && selectedProduct.media.length > 0 && (
                            <>
                                <Divider orientation="left" className="!text-xs !uppercase !tracking-widest !text-gray-400">{t('products.catalog.drawer.gallery')}</Divider>
                                <div className="flex gap-3 flex-wrap">
                                    {selectedProduct.media.map(m => (
                                        <div key={m.id} className="relative">
                                            <img src={m.url} alt="" className="w-20 h-20 object-cover rounded border border-gray-100 dark:border-gray-900" />
