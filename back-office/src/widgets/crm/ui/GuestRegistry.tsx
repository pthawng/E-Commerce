  ...
  const columns = [
    {
      ...
      render: (_: any, record: any) => (
        <Space size={12}>
          ...
          <Avatar
            icon={<GlobalOutlined />}
            className="bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-500"
          />{" "}
          <div>
            {" "}
            <Text strong className="block text-sm font-serif">
              {maskPII(record.email, "email")}
            </Text>{" "}
            <Text className="text-[9px] text-gray-400 uppercase tracking-widest">
              {t("crm.guests.table.guest")}
            </Text>{" "}
          </div>{" "}
        </Space>
      ),
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        return (
          <Tag
            color={isVIP ? "gold" : isLoyal ? "blue" : "default"}
      ...
      render: (_: any, record: any) => (
        ...
        >
          ...
          <Space size={4} className="text-[10px] font-bold text-gray-400">
            {" "}
            <ShoppingOutlined className="text-gray-300" /> {record.orderCount}{" "}
            {t("common.orders")}{" "}
          </Space>{" "}
          <br />{" "}
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Text className="text-[10px] text-gray-400 uppercase tracking-widest">
          {" "}
          {record.lastOrderAt
            ? new Date(record.lastOrderAt).toLocaleDateString()
            : "—"}{" "}
        </Text>
      ),
    },
    ...
    <div className="space-y-6">
      ...
      <Table
        ...
        pagination={{
          current: page,
          pageSize: 15,
          total: guests?.meta?.totalItems || 0,
          onChange: setPage,
