...
export const GuestOrdersModal: React.FC<GuestOrdersModalProps> = memo(({ email, open, onClose }) => {
    ...
    return (
        <Modal
            title={
                <Space>
                    ...
                    <span className="font-serif">{t('crm.guests.table.view_orders')}</span>
                </Space>
            }
            open={open}
            onCancel={onClose}
        ...
        >
            <div className="mb-6 p-4 bg-amber-50/30 border border-amber-100/50 dark:bg-amber-900/10 dark:border-amber-900/30">
                <Text className="text-[10px] uppercase tracking-widest text-gray-400 font-bold block mb-1">{t('orders.drawer.customer')}</Text>
            ...
            <Table
                ...
                columns={[
                    ...
                    },
                    {
                        ...
                        key: 'totalAmount',
                        render: (v: number) => <Text className="font-serif text-xs text-amber-600">{convertAndFormat(v)}</Text>
                    },
                    {
                        title: t('orders.drawer.date'),
                        dataIndex: 'createdAt',
                        key: 'createdAt',
                        render: (v: string) => <Text className="text-[10px] text-gray-400 font-bold">{new Date(v).toLocaleDateString()}</Text>
                    }
                ]}
