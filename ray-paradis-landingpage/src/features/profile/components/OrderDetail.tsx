...
export const OrderDetail: React.FC = () => {
  ...
  if (error || !order) {
    return (
      <div className="bg-surface-container-lowest py-32 px-10 text-center shadow-luxury flex flex-col items-center justify-center animate-in fade-in duration-1000">
        ...
        <h2 className="text-3xl font-display italic text-primary/80 mb-6">Manifest Not Found</h2>
        <p className="text-muted-foreground font-body text-sm max-w-sm mx-auto mb-12 leading-relaxed">
          The requested acquisition manifest could not be retrieved from our archives.
        </p>
        <Link 
          to="/account/orders"
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string, color: string }> = {
      pending: { label: 'Pending Authentication', color: 'text-gold-light border-gold-light/20 bg-gold-shimmer/10' },
      confirmed: { label: 'Confirmed', color: 'text-gold border-gold/30 bg-gold-shimmer/20' },
      processing: { label: 'In Preparation', color: 'text-primary/70 border-primary/20 bg-primary/5' },
      shipping: { label: 'In Transit', color: 'text-blue-500 border-blue-500/20 bg-blue-500/5' },
      delivered: { label: 'Delivered', color: 'text-emerald-500 border-emerald-500/20 bg-emerald-500/5' },
      completed: { label: 'Archived', color: 'text-primary/40 border-primary/10' },
      cancelled: { label: 'Cancelled', color: 'text-destructive border-destructive/20 bg-destructive/5' },
      failed: { label: 'Failed', color: 'text-destructive border-destructive/20 bg-destructive/5' },
    };
      ...
      <AnimatePresence>
        {order.status === 'failed' && (
          ...
          >
            ...
            <div>
              <p className="text-sm font-body text-destructive font-medium mb-1 uppercase tracking-widest">Transaction Failed</p>
              <p className="text-xs text-destructive/70 leading-relaxed italic">
                {order.cancelReason || "We were unable to process your payment. Please review the details below or initiate a new transaction protocol."}
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
        ...
        <div className="lg:col-span-8 space-y-16">
          <section className="space-y-8">
            <div className="flex items-center justify-between border-b border-primary/5 pb-4">
              <h3 className="text-[10px] uppercase tracking-ultra text-primary/40">Manifest Items</h3>
            </div>
          ...
          <section className="space-y-12">
            <div className="flex items-center justify-between border-b border-primary/5 pb-4">
              <h3 className="text-[10px] uppercase tracking-ultra text-primary/40">Acquisition Timeline</h3>
            </div>
        ...
        <aside className="lg:col-span-4 space-y-8">
          ...
          <div className="p-8 border border-primary/5 bg-primary/[0.02]">
            <h4 className="text-[10px] uppercase tracking-ultra text-primary/60 mb-6">Shipping Destination</h4>
