...
export async function seedCRM(prisma: PrismaClient) {
  ...
  for (let i = 0; i < 1000; i += CHUNK_SIZE) {
    const chunkPromises = Array.from({ length: Math.min(CHUNK_SIZE, 1000 - i) }).map(
      async (_, idx) => {
  ...
  const variants = await prisma.productVariant.findMany({
    select: { id: true, price: true, sku: true, variantTitle: true },
  });
  ...
  for (let i = 0; i < 2000; i += CHUNK_SIZE) {
    const chunkPromises = Array.from({ length: Math.min(CHUNK_SIZE, 2000 - i) }).map(
      async (_, idx) => {
        const globalIdx = i + idx;
        ...
        const selectedVariants = Array.from({ length: itemsCount }).map(
                ...
                create: [
                  {
                    ...
                    description: 'Order initiated by customer',
                    createdAt,
                    actorType: 'customer',
                  },
                  ...(isPaid
    ...
    if ((i + CHUNK_SIZE) % 500 === 0)
      console.log(`...Simulated ${i + CHUNK_SIZE}/2000 life cycles`);
  }
