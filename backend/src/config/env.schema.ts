...
export const envSchema = z
  .object({
    ...
    BACK_OFFICE_MFA_JWT_SECRET: z.string().min(32).optional(),
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.NODE_ENV === 'production' && !value.BACK_OFFICE_MFA_JWT_SECRET) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'BACK_OFFICE_MFA_JWT_SECRET is required in production',
        path: ['BACK_OFFICE_MFA_JWT_SECRET'],
      });
    }

    if (value.NODE_ENV === 'production' && !value.MFA_ENCRYPTION_KEY) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'MFA_ENCRYPTION_KEY is required in production',
