...
export class LedgerService {
  ...
  async getMaterialBalances() {
    ...
    return materialItems.map((item) => {
      ...
      const displayName =
        nameObj?.vi || nameObj?.en || Object.values(nameObj || {})[0] || 'Unknown Material';

      const variantTitleObj = item.productVariant?.variantTitle as any;
      const displayVariantTitle =
        variantTitleObj?.vi || variantTitleObj?.en || Object.values(variantTitleObj || {})[0] || '';
  ...
  async runForensicAudit() {
    ...
    for (const order of orders as any[]) {
      const totalPaid = order.transactions.reduce(
