...
export class CircuitBreakerService {
  ...
  private getStateKey(provider: string) {
    return `cb:state:${provider}`;
  }
  private getSuccessKey(provider: string) {
    return `cb:success:${provider}`;
  }
  private getFailKey(provider: string) {
    return `cb:fail:${provider}`;
  }
  private getOpenTimestampKey(provider: string) {
    return `cb:open_time:${provider}`;
  }
    ...
    if (state === CircuitBreakerState.OPEN) {
      ...
      if (openTimeStr) {
        ...
        if (now - openTime > this.COOLDOWN_SECONDS * 1000) {
          ...
          this.logger.log(
            `CircuitBreaker [${provider}]: Cooldown finished. Transitioning OPEN -> HALF_OPEN`,
          );
  ...
  async recordSuccess(provider: string) {
    ...
    if (state === CircuitBreakerState.HALF_OPEN) {
      ...
      await pipeline.exec();
      this.logger.log(
        `CircuitBreaker [${provider}]: Recovery successful. Transitioning HALF_OPEN -> CLOSED`,
      );
      return;
    }
  ...
  async recordFailure(provider: string) {
    ...
    if (state === CircuitBreakerState.HALF_OPEN) {
      ...
      this.logger.warn(
        `CircuitBreaker [${provider}]: Recovery failed. Transitioning HALF_OPEN -> OPEN`,
      );
      return;
    }
    ...
    if (results && results.length === 2 && !results[0][0] && !results[1][0]) {
      ...
      if (total >= this.MIN_REQUESTS) {
        ...
        if (errorRate > this.ERROR_THRESHOLD_PERCENT) {
          ...
          await this.redis.set(this.getOpenTimestampKey(provider), now);
          this.logger.error(
            `CircuitBreaker [${provider}]: Error rate ${errorRate.toFixed(1)}% (${failCount}/${total}) exceeded threshold. Transitioning CLOSED -> OPEN`,
          );
        }
      }
    }
