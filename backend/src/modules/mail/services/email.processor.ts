...
export class EmailProcessor {
  ...
  async handleSendEmail(job: Job<any>) {
    const { outboxId, recipient, subject, templateName, templateVersion, context, attempts } = job.data;
    ...
    try {
      ...
      let result = await this.sendWithFallback(recipient, subject, html, outboxId);
      ...
      this.logger.error(`Fatal error in EmailProcessor for outboxId ${outboxId}: ${error.message}`, error.stack);
  ...
  private async sendWithFallback(to: string, subject: string, html: string, outboxId: string) {
    ...
    if (sendGridState !== CircuitBreakerState.OPEN) {
      ...
      const sgResult = await this.sendgridProvider.send({ to, subject, html, metadata: { outboxId } });
      ...
      this.logger.warn(`Primary provider (SendGrid) failed for ${outboxId}. Falling back to SES...`);
    } else {
      this.logger.warn(`Circuit Breaker for SendGrid is OPEN. Directing traffic to SES for ${outboxId}.`);
    }
