...
/**
 * Listener that handles product media anomaly reports.
 * Detects anomalies and automates mitigation steps like hiding products and notifying admins.
 */
@Injectable()
export class ProductAnomalyListener {
  private readonly logger = new Logger(ProductAnomalyListener.name);
  private readonly THRESHOLD = 1; // Threshold set to 1 for testing; production standard is 3+
  ...
  async handleMediaAnomaly(payload: { productId: string; mediaUrl: string; clientId: string }) {
    ...
    this.logger.debug(
      `[Listener] Received anomaly report for product ${productId} from client ${clientId}`,
    );
    const cacheKey = `anomaly:product:media:${productId}`;

    // Get existing reports
    const reports: string[] = (await this.cacheManager.get(cacheKey)) || [];
