...
export abstract class BasePaymentProvider implements IPaymentGatewayProvider {
   ...
   */
  async verifyCallback(callbackData: Record<string, any>): Promise<CallbackData> {
  ...
  async processRefund(
    ...
    try {
      ...
      this.logger.log(`Refund processed successfully: ${result.refundTransactionId}`);
      return result;
    } catch (error) {
      this.logger.error(
  ...
  protected abstract doVerifyCallback(callbackData: Record<string, any>): Promise<CallbackData>;
