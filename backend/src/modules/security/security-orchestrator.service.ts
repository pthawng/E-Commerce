...
import { SecurityEventBus, SecurityEventType, SecurityEvent } from './security-event-bus.service';
...
export class SecurityOrchestrator implements OnModuleInit {
    ...
    private async handleEvent(event: SecurityEvent) {
        this.logger.warn(`Security Event Captured: [${event.type}] for Principal [${event.principal.id}]`);
    ...
    private async handleTokenReplay(event: SecurityEvent) {
        ...
        this.logger.error(`CRITICAL: Triggering Global Revocation for User ${event.principal.id} due to TOKEN_REPLAY`);
    ...
    private async handleOwnershipViolation(event: SecurityEvent) {
        ...
        this.logger.warn(`Behavioral Alert: Principal ${event.principal.id} attempted unauthorized access to ${event.metadata?.context}`);
    }
}
