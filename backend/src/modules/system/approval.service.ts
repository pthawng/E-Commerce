...
export class ApprovalService {
   ...
   * Captures intent, original state, and required authority.
   */
  ...
  async approve(requestId: string, approverId: string, comment?: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      // Enforce the four-eyes principle where the approver cannot be the requestor
      if (request.requestorId === approverId) {
        throw new BadRequestException(
          '4-Eyes Principle Violation: You cannot approve your own request.',
        );
      }
