...
export class CurrencyService {
    ...
    constructor(
        ...
        this.API_URL = this.configService.get<string>('CURRENCY_API_URL', 'https://api.frankfurter.app/latest?from={base}&to={target}');
    }
    ...
    async getRate(targetCurrency: string): Promise<number> {
        ...
        try {
            ...
            this.logger.warn(`External API fetch failed for ${targetCurrency}: ${error.message}. Falling back to DB.`);
        }
    ...
    private buildRateUrl(targetCurrency: string): string {
        if (this.API_URL.includes('{base}') || this.API_URL.includes('{target}')) {
            return this.API_URL
                .replace('{base}', encodeURIComponent(this.BASE_CURRENCY))
