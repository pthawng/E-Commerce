...
import { API_ENDPOINTS, API_BASE_URL, type AuthResponse, type User } from '@shared';
...
export function AuthSheet({ open, onOpenChange }: AuthSheetProps) {
  ...
  const handleGoogleLogin = () => {
    ...
    const startUrl = `${API_BASE_URL.replace(/\/$/, '')}/api/auth/oauth/google/start?returnTo=${encodeURIComponent(returnTo)}`;
              ...
              {mode !== 'forgot' && (
                <div className="mt-8 space-y-6">
                  <div className="relative flex items-center justify-center">
                    ...
                    <span className="relative px-3 text-3xs text-muted-foreground/60 font-body uppercase tracking-widest bg-background">
                      {t('auth.links.orWith')}
                    </span>
                  </div>
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="w-full h-12 text-2xs tracking-ultra border-muted/50 hover:bg-muted/10 flex items-center justify-center gap-3 font-body"
                  ...
                  >
                    ...
                    <span>{t('auth.links.continueWithGoogle')}</span>
                  </Button>
                </div>
              )}
            </motion.div>
